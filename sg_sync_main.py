...
def normalize_sg_name(name: str, src_acct: str = "", tgt_acct: str = "") -> str:
    """
    Safe enterprise SG-name normalization.

    Goals:
    - Match CloudFormation/random suffix variants.
    - Preserve meaningful application words like datasync, hnc, ava, ec2servers.
    - Avoid collapsing names too aggressively.
    """
    original = name or ""
    normalized = original.strip()

    # Replace embedded account ID only when both are known and different.
    if src_acct and tgt_acct and src_acct != tgt_acct:
        normalized = normalized.replace(src_acct, tgt_acct)

    normalized = normalized.lower().strip()

    # Handle CloudFormation stack pattern while keeping the logical resource section.
    # Example:
    # stack-app-nested-rsecuritygroup-1abc2345 -> rsecuritygroup
    cfn_match = re.match(
        r"^stack-[a-z0-9][a-z0-9-]*-([a-z0-9-]*(?:securitygroup|sg)[a-z0-9-]*)-(?:[a-f0-9]{8,}|(?=[a-z0-9]*\d)[a-z0-9]{8,})$",
        normalized,
        flags=re.IGNORECASE,
    )
    ...
    if cfn_match:
        ...
        # Only strip a stack prefix when the remaining name still looks SG-related.
        maybe = re.sub(
            r"^stack-[a-z0-9][a-z0-9-]*-",
            "",
            normalized,
            flags=re.IGNORECASE,
        )
        if maybe != normalized and re.search(r"(securitygroup|sg)", maybe, flags=re.IGNORECASE):
            normalized = maybe

    # Strip environment + random prefix.
    # Example:
    # d-afshcjcjv-sg-name -> sg-name
    # dr-afshcjcjv-sg-name -> sg-name
    normalized = re.sub(
    ...
    # Strip leading account-number prefix.
    # Example: 123456789012-sg-name -> sg-name
    normalized = re.sub(
        r"^[0-9]{6,12}[-_]",
        "",
        normalized,
    ...
    # Strip trailing generated suffix ONLY when it looks random/hash-like.
    # This avoids breaking names like:
    # prxix-prod-datasync
    #
    # Good strip:
    # sg-name-1rsc3wy00p7ht -> sg-name
    # sg-name-a1b2c3d4 -> sg-name
    #
    # Do NOT strip:
    # prxix-prod-datasync
    # pr_cw_synthetics_to_ec2servers_https_20080
    normalized = re.sub(
    ...
    # Collapse duplicate separators but preserve single underscores and dashes.
...
def index_target_sgs(target_sgs: List[Dict[str, Any]], src_acct: str, tgt_acct: str) -> Dict[str, Dict[str, List[Dict[str, Any]]]]:
    indexes = {"by_exact_name": defaultdict(list), "by_normalized": defaultdict(list), "by_cfn": defaultdict(list)}
    for sg in target_sgs:
        name = sg.get("GroupName", "")
        indexes["by_exact_name"][name].append(sg)
        indexes["by_normalized"][normalize_sg_name(name, src_acct, tgt_acct)].append(sg)
        cfn_key = cfn_match_key(sg)
        if cfn_key:
...
def match_source_to_target(source_sg: Dict[str, Any], indexes, name_map: Dict[str, str], src_acct: str, tgt_acct: str) -> MatchInfo:
    ...
    if source_name in name_map:
        ...
        if target:
            return MatchInfo(source_name, source_id, normalized_key, target.get("GroupName"), target.get("GroupId"), "manual-name-map", "manual")
        return MatchInfo(source_name, source_id, normalized_key, match_method="manual-name-map", match_confidence="unmatched", ambiguous=ambiguous, ambiguity_reason=reason or f"Manual name-map target not found: {target_name}")

    target, ambiguous, reason = get_single_match(indexes["by_exact_name"].get(source_name, []))
    if target:
        return MatchInfo(source_name, source_id, normalized_key, target.get("GroupName"), target.get("GroupId"), "exact-name", "exact")
    if ambiguous:
        return MatchInfo(source_name, source_id, normalized_key, match_method="exact-name", match_confidence="ambiguous", ambiguous=True, ambiguity_reason=reason)

    source_cfn_key = cfn_match_key(source_sg)
    if source_cfn_key:
        target, ambiguous, reason = get_single_match(indexes["by_cfn"].get(source_cfn_key, []))
        if target:
            return MatchInfo(source_name, source_id, normalized_key, target.get("GroupName"), target.get("GroupId"), "cloudformation-tags", "tag-based")
        if ambiguous:
            return MatchInfo(source_name, source_id, normalized_key, match_method="cloudformation-tags", match_confidence="ambiguous", ambiguous=True, ambiguity_reason=reason)

    target, ambiguous, reason = get_single_match(indexes["by_normalized"].get(normalized_key, []))
    if target:
        confidence = "low" if is_low_confidence_key(normalized_key) else "normalized"
        return MatchInfo(source_name, source_id, normalized_key, target.get("GroupName"), target.get("GroupId"), "normalized-name", confidence)
    if ambiguous:
        return MatchInfo(source_name, source_id, normalized_key, match_method="normalized-name", match_confidence="ambiguous", ambiguous=True, ambiguity_reason=reason)
    ...
    for sg in source_groups:
        ...
        if not match.target_group_id:
            possible_same_key = [
                ...
                if normalize_sg_name(
                    candidate_sg.get("GroupName", ""),
                    src_acct,
                    tgt_acct,
                ) == match.normalized_match_key
            ]
