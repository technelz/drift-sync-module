...
def main() -> int:
    ...
    if args.source_profile and args.source_region:
        ...
        # JSON mode: infer source prefix list IDs only if they are AWS-managed and known in target-style replacement later
        # Better to supply live source profile when prefix lists are involved, but we still build target maps below.
        source_prefix_id_to_name = {}
    ...
    # Fallback: infer source prefix list names from source rules if source profile wasn't provided.
    # This works only when prefix list names are already visible in target naming style or not region-qualified.
            ...
            for perm in (sg.get("IpPermissions") or []) + (sg.get("IpPermissionsEgress") or []):
                for p in perm.get("PrefixListIds", []) or []:
                    ...
                    if plid and plid not in source_prefix_id_to_name:
                        # Unknown name; leave unresolved unless source live profile is supplied.
